    ...
    public void onInteract(PlayerInteractEvent event) {
        ...
        if (event.getAction() == Action.RIGHT_CLICK_AIR || event.getAction() == Action.RIGHT_CLICK_BLOCK) {
            ...
            if (profile.getCooldown(LuckyIngot.class).onCooldown(player)) {
                player.sendMessage(CC.translate("&fYou are on &c&lLucky Ingot &7cooldown for &4" + DurationFormatter.getRemaining(profile.getCooldown(LuckyIngot.class).getRemainingMillis(player), true, true)));
